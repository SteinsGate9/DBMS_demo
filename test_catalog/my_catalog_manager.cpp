...
bool Catalog_Manager::AddIndex(string index_name,string table_name,string col_name)
{
	...
	if(pos==tables.end())
	{
		cout<<"Fail to create index, table "<<table_name<<" does not exist"<<endl;
		return 0;
	}
	if(flag == 0)
	{
...
bool Catalog_Manager::AddTable(string table_name,vector<string> col_name,vector<string> col_type,vector<bool> col_unique)
{
	string pri("no");
...
//在创建表时，向字典信息中增加一个表的信息
bool Catalog_Manager::AddTable(string table_name,vector<string> col_name,vector<string> col_type,vector<bool> col_unique,string pri)
{
	vector<Table>::iterator pos;
	for(pos=tables.begin();pos!=tables.end();++pos)
	{
